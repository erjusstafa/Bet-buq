...
function Casino() {
  ...
  let displayNameCateg = useSelector(
    (state) => state?.betbuqsport?.CasinoData?.result
  );
  ...
  let categories = Object.values(displayNameCateg?.categories || {})
    .filter((F) =>
      allConfig["betConstructWidget"]
        ? !allConfig.dontshowCateg.includes(F.id)
        : null
    )
    .map((C) => (
      <p
        className={
          idLink.link === C.id && C.name === idLink.activeText ? "active" : ""
        }
        key={C.id}
        ...
        <div style={{ background: "#313d42" }}>
          <div className="link">
            <div className="link-live">
              <span
                onClick={() => changeId(8888)}
                className={"heart " + (idLink.other === 8888 ? "active" : "")}
              >
                <i className={heartIcon} />
              </span>
              <p
                onClick={() => changeId(9999, idLink.activeText)}
                className={idLink.other === 9999 ? "active" : ""}
              >
                All Games
              </p>
              {categories ? categories : null}
              <p
                onClick={() => changeId(7777, idLink.activeText)}
                className={idLink.other === 7777 ? "active" : ""}
              >
                Providers
              </p>
            </div>
...
class CasinoItemGames extends Component {
  ...
  render() {
    return Object.values(this.props.displayNameCateg?.providers || {}).map(
      (H, index) => (
        <Fragment key={index}>
                    ...
                    <div className="images-name">
                      ...
                      <span>
                        ...
                        <i
                          ...
                          style={{
                            color: `${!this.state.colorFav ? "" : "#1ab7af"}`,
                          }}
                        />
                      </span>
                    </div>
